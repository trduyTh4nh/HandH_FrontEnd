...
export default function HomeCategory(props: HomeCategoryWidgetProps) {
  return (
    <Link
      to={`/shopcate?cate=${props.id}`}
      className={`flex group w-full flex-col gap-2 overflow-hidden cursor-pointer no-underline ${props.className}`}
    >
      ...
      <h3 className="text-center text-2xl font-bold no-underline">
        {props.name}
      </h3>
    </Link>
  );
